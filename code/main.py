...
active.active_force['A'] = (mag/np.sqrt(3),0,0)
active.active_torque['A'] = (0,0,0)
#diffusion = active.create_diffusion_updater(trigger=1, rotational_diffusion=0.1)
...
outbreak = rereorient.reorient(numActive = nA, cone = cone, door= exit_width) #Change orientation to look towards the door
outbreak_op = hoomd.update.CustomUpdater(
    action=outbreak, 
...
hoomdini = hoomdini.hoomdini(numActive = nA, numEscaped = nE)
hoomdini_op = hoomd.update.CustomUpdater(
    action = hoomdini,
    trigger=hoomd.trigger.Periodic(int(1)))

sim.operations.writers.append(status_op)
sim.operations += hoomdini_op
sim.operations += outbreak_op
